        ...
        for context_dept_detail in context_dept_detail_list:
            if context_dept_detail.res_dept_id not in dept_id_list:
                self.__dept_service.update_dept(
                    dept_info=DeptModel(
                        id=context_dept_detail.dept_id,
                        version=context_dept_detail.dept_version,
                        name=context_dept_detail.name,
                        organization_id=context_dept_detail.organization_id,
                        start_at=context_dept_detail.start_at,
                        finish_at=finish_at,
                    ),
                    transaction=transaction,
                ...
                self.__dimension_dept_tree_repository.update_dimension_dept_tree(
                    data=DimensionDeptTreeModel(
                        id=context_dept_detail.dimension_dept_tree_id,
                        version=context_dept_detail.dimension_dept_tree_version,
                        dept_id=context_dept_detail.dept_id,
                        parent_dept_id=context_dept_detail.parent_dept_id,
                        finish_at=finish_at,
                    ),
                    transaction=transaction,
                    limited_col_list=["finish_at"],
