  ...
  export default defineComponent({
    ...
    setup(props, { expose, emit }) {
      ...
      const getImageStyle = computed(() => {
        const { imgScale, imgRotate, imgTop, imgLeft, width, height } = imgState;
        let res = {
        ...
        // if (!inited) {
        //   res['transform'] = `scale(1}) rotate(${imgRotate}deg)`;
        //   // eslint-disable-next-line vue/no-side-effects-in-computed-properties
        //   imgState.inited = true;
        // }
        if (width > 0 && height > 0) {
          ...
          if (width > clientWidth) {
            widthVal = clientWidth * 0.8;
          }
          let heightVal = height;
          if (height > clientHeight) {
            heightVal = clientHeight * 0.8;
          }
          res['maxWidth'] = `${widthVal}px`;
          res['maxHeight'] = `${heightVal}px`;
          // if (width > height) {
          //   res['maxWidth'] = `${widthVal}px`;
          // } else {
          //   res['maxHeight'] = `${heightVal}px`;
