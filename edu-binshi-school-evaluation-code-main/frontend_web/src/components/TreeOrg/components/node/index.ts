...
function createListener(handler: any, data: INode) {
  if (typeof handler === 'function') {
    return function (e: MouseEvent) {
      ...
      if (className.indexOf('org-tree-node-btn') > -1) return;
      handler(e, data);
    };
  }
}
...
export const renderLabel = (h: any, data: INode, context: any, root: boolean) => {
  ...
  if (context.slots.default) {
    ...
    log.warning(
      'scoped-slot header is easier to use. We recommend users to use scoped-slot header.',
    );
  ...
  selectedClassName &&
    selectedKey &&
    selectedKey.includes(data[defaultProps.id]) &&
  ...
  if (attrs.vNodedrag && vNodedrag && !root) {
    cloneDirs.push([
      vNodedrag,
      Object.assign({ node: data }, attrs.vNodedrag),
      '',
      { l: true, t: true },
    ]);
  }
      ...
      withDirectives(
        h('textarea', {
          ...
          onInput: (event: InputEvent) => {
            data[defaultProps.label] = (<HTMLInputElement>event.target).value;
          },
          onBlur: (e: MouseEvent) => {
            data.focused = false;
