...
export const useDraggable = (
  ...
  function elementDown(e: MouseEvent | TouchEvent) {
    ...
    if (target && refs.eleRef.value && refs.eleRef.value.contains(target)) {
      if (
        props.dragCancel &&
        matchesSelectorToParentElements(target, props.dragCancel, refs.eleRef.value)
      ) {
  ...
  function calcDragLimits() {
    ...
    return {
      ...
      maxTop:
        Math.floor((parentHeight.value - height.value - top.value) / grid[1]) * grid[1] + top.value,
      minBottom: bottom.value % grid[1],
      maxBottom:
        Math.floor((parentHeight.value - height.value - bottom.value) / grid[1]) * grid[1] +
        bottom.value,
    };
  }
  ...
  function handleDrag(e: MouseEvent | TouchEvent) {
    ...
    const rt = restrictToBounds(
      mouseClickPosition.right + deltaX,
      bounds.minRight,
      bounds.maxRight,
    );
    const bt = restrictToBounds(
