...
//       parentData.children
//         ? parentData.children.push(nodeClone)
//         : (parentData.children = [].concat(nodeClone));
//     } else {
//       // 如果拷贝并拖拽节点
...
//       parentData.children
//         ? parentData.children.push(nodeClone)
//         : (parentData.children = [nodeClone]);
//     }
//   }
// };
let vm;
export const getThis = (that) => {
  vm = that;
};
let vmProps;
export const getProps = (that) => {
  vmProps = that;
};
// 拖拽排序
const dragToSort = function (node: INode, context: IContext) {
  const { parenNode, keys } = context;
  if (node && parenNode.value) {
    if (node.pid !== parenNode.value.pid) {
      ...
      const parent = getNodeById(vm.treeData, keys, node.pid);
      const nodeIndex = parent.children.map((x) => x.id).indexOf(node.id);
      const parenNodeIndex = parent.children.map((x) => x.id).indexOf(parenNode.value.id);
      parent.children.splice(nodeIndex, 1);
      parent.children.splice(parenNodeIndex, 0, node);
    }
  }
};
...
const drag: ObjectDirective = {
  beforeMount(el: HTMLElement, binding: DirectiveBinding) {
    ...
    let screenX = 0;
    let screenY = 0;
    function initData(e: MouseEvent) {
      // 初始化拖动数据
      screenX = e.screenX;
      screenY = e.screenY;
      offsetLeft = 0;
    ...
    function handleDownCb(e: MouseEvent) {
      ...
      if (Array.isArray(target.className)) {
      ...
      if (
        ...
        node.focused ||
        node.noDragging ||
        className.indexOf('tree-org-node-btn') > -1
      ) {
        return false;
      }
      initData(e);
    ...
    function handleUpCb() {
      ...
      //   return false;
      // }
      dragToSort(node, instance as IContext);
