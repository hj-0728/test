  ...
  export default defineComponent({
    ...
    setup(props, { attrs, slots, emit, expose }) {
      ...
      const state = reactive<TreeState>({
        ...
        deleteDefaultHeight: props.deleteDefaultHeight,
      });
      ...
      const getNotFound = computed((): boolean => {
        return (!getTreeData.value || getTreeData.value.length === 0) && !props.loading;
      });
      ...
      return () => {
        const { title, helpMessage, toolbar, search, checkable, deleteDefaultHeight } = props;
        const showTitle = title || toolbar || search || slots.headerTitle;

        let scrollStyle: CSSProperties = { height: 'calc(100% - 38px)' };
        if (deleteDefaultHeight) {
          scrollStyle = {};
        }
        return (
          <div class={[bem(), 'h-full', attrs.class]}>
            ...
            <Loading loading={loading.value} absolute={true} />
