...
from infra_dingtalk.dingtalk.core.exceptions import DingTalkClientException
from infra_dingtalk.dingtalk.core.utils import json_loads
from infra_dingtalk.dingtalk.storage.memorystorage import MemoryStorage
...
class BaseClient(object):
    ...
    def _request(self, method, url_or_endpoint, **kwargs):
        ...
        result = self._handle_result(res, method, url, result_processor, top_response_key, **kwargs)
    ...
    def _handle_result(
        self, res, method=None, url=None, result_processor=None, top_response_key=None, **kwargs
    ):
        ...
        **kwargs,
        ...
        if params is not None:
            for key, value in params.items():
                reqparams[key] = (
                    value if not isinstance(value, (dict, list, tuple)) else json.dumps(value)
                )
        ...
        try:
            return self._request(
                "POST", base_url, params=reqparams, top_response_key=response_key, **kwargs
            )
        except DingTalkClientException as e:
            return self._handle_request_except(
                e, self.request, method, format_, v, simplify, partner_id, url, params, **kwargs
            )
