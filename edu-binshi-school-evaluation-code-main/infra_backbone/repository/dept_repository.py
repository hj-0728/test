...
class DeptRepository(BasicRepository):
    ...
    def get_dept_tree(
        ...
        if is_available is not None:
            sql += """ and is_available is :is_available"""
    ...
    def get_dept_by_id(self, dept_id: str) -> Optional[DeptModel]:
        """
        根据id获取部门
        :param dept_id:
        :return:
        ...
        sql = """select * from st_dept where id = :dept_id"""
        return self._fetch_first_to_model(model_cls=DeptModel, sql=sql, params={"dept_id": dept_id})
    ...
    def get_dept_info(
        ...
        SELECT COUNT((st2.is_available IS FALSE) OR NULL) AS parent_not_available_count
        FROM sv_dimension_dept_tree st
    ...
    def get_current_dept_by_ids(self, dept_ids: List[str]) -> List[DeptModel]:
        ...
        where id=any(array[:dept_ids]) and finish_at>now()
        """
