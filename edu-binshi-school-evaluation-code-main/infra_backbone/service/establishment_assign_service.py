...
class EstablishmentAssignService:
    ...
    def add_establishment_assign(
        ...
        for establishment_assign in establishment_assign_list:
            if not establishment_assign.dimension_dept_tree_id:
                dimension_dept_tree = self.__dimension_dept_tree_service.get_dimension_dept_tree_by_dept_id_and_dimension(
                    dimension_category=establishment_assign.dimension_category,
                    dept_id=establishment_assign.dept_id,
                    organization_id=establishment_assign.organization_id,
                )
                establishment_assign.dimension_dept_tree_id = dimension_dept_tree.id
            if establishment_assign.establishment_id:
                establishment_id = establishment_assign.establishment_id
            elif establishment_dict.get(
                f"{establishment_assign.dimension_dept_tree_id}&&{establishment_assign.capacity_code}"
            ):
                establishment_id = establishment_dict.get(
                    f"{establishment_assign.dimension_dept_tree_id}&&{establishment_assign.capacity_code}"
                )
            else:
                establishment = self.__establishment_service.fetch_establishment_info(
                    capacity_name=establishment_assign.capacity_name,
    ...
    def update_people_establishment_assign(
        ...
        for exist_establishment_assign in exist_establishment_assign_list:
            if not exist_assign_dict.get(
                ...
                self.__establishment_assign_repository.delete_establishment_assign_by_id(
                    establishment_assign_id=exist_establishment_assign.id, transaction=transaction
                )
        ...
        for k, v in exist_assign_dict.items():
            if not establishment_assign_dict.get(k):
                v.finish_at = handle_time
                self.__establishment_assign_repository.update_establishment_assign(
                    data=v.cast_to(EstablishmentAssignModel),
                    transaction=transaction,
