...
from backend.utility.server_helper import prepare_url
...
def process_init_func():
    init = current_app.config.get("INIT")
    if not init:
        init_app_process()
        current_app.config["INIT"] = True
...
def process_before_request():
    ...
    try:
        ...
        exclude_route_list = [
            '/api/web/site-message/unread-exist'
        ]
...
def _add_access_log(
    ...
    if isinstance(data, dict) and len(file_info_list) > 0:
        data["file_info_list"] = file_info_list
    ...
    command = CommandEm(category="task_save_access_log", args=access_log)
...
def init_app_process(
    ...
    with uow:
        ...
        authorization_service.prepare_route(transaction=transaction)
        # 加注路由
...
def _check_route_authorization(
    ...
    def __process_need_authed():
        ...
        user_service.check_user_profile_by_redis()
...
def _check_master_user_role(
    ...
    current_role_id = app_role_service.get_current_role_id()
    if current_role_id not in role_id_list:
        raise NotPermissionError()


@inject
def _check_master_user_ability_permission(
    ability_permission_id_list: List[str],
    app_ability_permission_service: AppAbilityPermissionService = Provide[
        BackendContainer.app_ability_permission_service
    ],
):
    """
    检查用户是否有权限
    :param ability_permission_id_list:
    :return:
    """

    master_user_ability_permission_list = (
        app_ability_permission_service.get_current_ability_permission_id_list()
    )

    match_list = set(ability_permission_id_list).intersection(master_user_ability_permission_list)
    if len(match_list) == 0:
        raise NotPermissionError()
