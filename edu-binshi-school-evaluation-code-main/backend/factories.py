...
def create_flask_app(
    ...
    # 配置babel，用于多国语言，默认使用中文
    app.config["BABEL_DEFAULT_LOCALE"] = "zh_Hans_CN"
    app.config["WTF_CSRF_ENABLED"] = False
    babel.init_app(app)
    # if babel_func and not babel.locale_selector_func:
    #     babel.localeselector(babel_func())

    # 开发环境数据配置
    dev_setting = DevSetting()
    app.config["DEV"] = dev_setting

    # 初始化pubsub配置
    pub_sub_setting = PubSubSettings()
    app.config["PUB_SUB_SETTING"] = pub_sub_setting
    ...
    if jwt_setting.blacklist_enabled:
        app.config["JWT_BLACKLIST_ENABLED"] = jwt_setting.blacklist_enabled
        app.config["JWT_BLACKLIST_TOKEN_CHECKS"] = jwt_setting.blacklist_token_checks
        app.config["JWT_REDIS_BLOCKLIST"] = Redis.from_url(
            url=jwt_setting.redis_blocklist, decode_responses=True
        )
