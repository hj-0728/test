...
from infra_dingtalk.dingtalk.client import api
from infra_dingtalk.dingtalk.client.base import BaseClient
from infra_dingtalk.dingtalk.core.utils import DingTalkSigner, random_string
from infra_dingtalk.dingtalk.crypto import DingTalkCrypto
from infra_dingtalk.dingtalk.storage.cache import DingTalkCache
...
class DingTalkClient(BaseClient):
    ...
    role = api.Role()
    user = api.User()
    k12_department = api.K12Department()
    k12_user = api.K12User()
    ...
    def access_token(self):
        ...
        if token is None:
            ...
            expires_in = ret.get("expires_in", 7199)
...
class SecretClient(DingTalkClient):
    def __init__(
        ...
        super(SecretClient, self).__init__(
            corp_id, "secret:" + corp_id, storage, timeout, auto_retry
        )
    ...
    def get_access_token(self):
        return self._request(
            "GET", "/gettoken", params={"corpid": self.corp_id, "corpsecret": self.corp_secret}
        )
...
class AppKeyClient(DingTalkClient):
    def __init__(
        ...
        super(AppKeyClient, self).__init__(
            corp_id, "secret:" + corp_id, storage, timeout, auto_retry
        )
        self.crypto = DingTalkCrypto(token, aes_key, corp_id)
    ...
    def get_access_token(self):
        return self._request(
            "GET", "/gettoken", params={"appkey": self.app_key, "appsecret": self.app_secret}
        )
