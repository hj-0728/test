...
class OrganizationRepository(BasicRepository):
    ...
    def fetch_activated_organization_list(self) -> List[OrganizationModel]:
        """
        获取已激活的组织列表
        :return:
        """
        sql = """select * from st_organization where is_activated is true"""
        return self._fetch_all_to_model(model_cls=OrganizationModel, sql=sql)
