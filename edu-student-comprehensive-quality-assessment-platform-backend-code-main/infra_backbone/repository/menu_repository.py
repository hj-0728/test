...
class MenuRepository(BasicRepository):
    ...
    def fetch_menu_list(
        self, menu_category: str, search_text: Optional[str]
    ) -> List[MenuViewModel]:
        """
        获取菜单树
        :param menu_category:
        ...
        """
        if search_text:
            sql = """ select sm2.* from sv_menu sm
            inner join sv_menu sm2 on sm2.id = any(sm.tree_id_list)
            where sm.category=:menu_category and sm.name like '%' || :search_text || '%'"""
        else:
            sql = """
            select * from sv_menu
        ...
        return self._fetch_all_to_model(
            model_cls=MenuViewModel,
            sql=sql,
            params={"menu_category": menu_category, "search_text": search_text},
        )
    ...
    def fetch_same_level_max_seq(self, parent_id: Optional[str]) -> int:
        """
        获取同级菜单最大的seq
        :param parent_id:
        :return:
        """
        sql = """
        select coalesce(max(seq), 0) as seq from st_menu
        """
        if parent_id:
            sql += """ where parent_id=:parent_id"""
        else:
            sql += """ where parent_id is null"""
        data = self._execute_sql(sql=sql, params={"parent_id": parent_id})
        return data[0]["seq"] + 1

    def fetch_menu_info(self, menu_id: str) -> Optional[MenuModel]:
        """
        获取菜单详情
        :param menu_id:
        :return:
