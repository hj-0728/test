...
class DictService:
    ...
    def get_dict_meta_by_meta_code(self, dict_meta_code: str) -> DictMetaModel:
        """
        根据字典元编码获取字典数据
        """
        dict_meta = self.__dict_repository.get_dict_meta_by_code(code=dict_meta_code)
        if not dict_meta:
            raise BusinessError(f"未获取到字典元数据，字典元编码：{dict_meta_code}")
        return dict_meta
