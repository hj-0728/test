...
class UserService:
    ...
    def reset_password(
        self, data: UserModel, transaction: Transaction, new_password: str = None
    ) -> str:
        """
        重置密码
        """
    ...
    def get_people_id_by_user_id(self, user_id: str) -> Optional[str]:
        """
        根据用户id获取人员id
        :param user_id:
        :return:
        """
        people_user = self.__people_user_repository.fetch_people_user_by_user_id(user_id=user_id)
        if not people_user:
            return None
        return people_user.people_id
    ...
    def get_user(self, user_id: str) -> UserModel:
        ...
        return self.__user_repository.get_user_by_id(user_id=user_id)
