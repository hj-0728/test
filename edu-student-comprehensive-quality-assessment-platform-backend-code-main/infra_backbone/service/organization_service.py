...
class OrganizationService:
    ...
    def get_system_unique_organization(self) -> OrganizationModel:
        """
        获取系统唯一组织
        :return:
        """
        organization_list = self.__organization_repository.fetch_activated_organization_list()
        if not organization_list:
            raise BusinessError("未获取到组织")
        if len(organization_list) > 1:
            raise BusinessError("系统中组织数大于1")
